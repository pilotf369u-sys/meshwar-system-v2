...
create table if not exists public.customer_review_login_attempts (
  id bigint generated by default as identity primary key,
  identity_hash bytea not null,
  attempted_at timestamptz not null default now()
);

create index if not exists idx_customer_review_login_attempts_window
  on public.customer_review_login_attempts(identity_hash, attempted_at desc);
create index if not exists idx_customer_review_login_attempts_cleanup
  on public.customer_review_login_attempts(attempted_at);
...
alter table public.customer_review_login_attempts enable row level security;
...
revoke all on public.customer_review_login_attempts from public, anon, authenticated;
