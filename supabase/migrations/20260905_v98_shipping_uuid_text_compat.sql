-- KINTO V98 hotfix 2 — align live orders TEXT identifiers with UUID tables.
-- Safe to run after the V98 shipping migration and the details type hotfix.
...
begin
  ...
  if coalesce((v_quote ->> 'profile_active')::boolean, false)
    ...
    raise exception 'SHIPPING_DESTINATION_UNSUPPORTED'
      using errcode = 'P0001';
  end if;
  ...
  if v_fee > 0
     and upper(coalesce(new.currency, '')) <> v_currency then
...
'V98 shipping trigger compatible with TEXT orders.details/customer_id and UUID customer/store tables.';
