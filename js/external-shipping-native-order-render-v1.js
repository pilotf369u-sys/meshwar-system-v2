...
const VERSION='20260829-native-render-v39';
...
function installAdmin(){
  ...
  window.adminOrderRow=function(o){
    const id=encodeURIComponent(String(o.id)),code=o.order_code||o.id,status=o.status||'انتظار رد الموظف',qty=o._quantity||adminOrderQuantity(o),price=product(o),url=adminOrderProductUrl(o),phone=o._customer_phone||o.customer_phone||'---',ext=localFee(o),extCur=String(o.currency||'USD'),due=collectionDue(o);
...
<td><div class="mw-native-controls"><input id="adminPrice-${id}" type="number" min="0" step="0.01" value="${esc(product(o))}" placeholder="0"><select id="adminCurrency-${id}" onchange="syncAdminExternalCurrency('${id}')">${adminShortCurrencyOptions(o.currency||'USD')}</select></div></td>
<td><div class="mw-native-controls"><input id="adminExternalFee-${id}" type="number" min="0" step="0.01" value="${esc(localFee(o))}" placeholder="0"><select id="adminExternalCurrency-${id}" data-manual-currency="0" onchange="markAdminExternalCurrencyManual('${id}')">${adminShortCurrencyOptions(extCur)}</select></div></td>
