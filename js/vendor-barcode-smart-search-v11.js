...
(function(){
  ...
  function isCurrentEdit(win,id){const d=win.document,modal=d.getElementById('productModal');return Boolean(modal&&!modal.classList.contains('hidden')&&String(d.getElementById('productId')?.value||'').trim()===String(id))}
  ...
  async function refreshProductDecorations(win,force=false){
    ...
    const now=Date.now();
    if(!force&&win.__mwBarcodeProductsCache&&now-(win.__mwBarcodeProductsCacheAt||0)<2500){decorateRows(win,win.__mwBarcodeProductsCache);return}
  ...
  function scheduleRefresh(win,force=false){clearTimeout(win.__mwBarcodeRefreshTimer);win.__mwBarcodeRefreshTimer=setTimeout(()=>refreshProductDecorations(win,force),120)}

  function injectUi(win){
    const d=win.document;
    if(!d.getElementById('productBarcode')){
      const name=d.getElementById('productName');
      if(name){const input=d.createElement('input');input.id='productBarcode';input.className='field';input.placeholder='الباركود';input.autocomplete='off';input.inputMode='numeric';name.insertAdjacentElement('afterend',input)}
    }
    ensureBarcodeColumn(win);
    const productsPanel=d.getElementById('vendorTab-products'),tableWrap=productsPanel?.querySelector('.vendor-table-wrap');
  ...
  function installFetchBridge(win){
    ...
    win.fetch=async function(input,init){
      try{
        ...
        if(/\/rest\/v1\/local_products(?:\?|$)/.test(url)&&['POST','PATCH'].includes(method)){win.__mwBarcodeProductsCacheAt=0;scheduleRefresh(win,true)}
  ...
  async function hydrateBarcodeForEdit(win,id){
    try{
      if(!isCurrentEdit(win,id))return;
      const sid=storeId(win);if(!sid)return;
      let p=(win.__mwBarcodeProductsCache||[]).find(x=>String(x.id)===String(id));
      if(!p){const rows=await rest(win,`local_products?select=id,barcode,stock_quantity&id=eq.${encodeURIComponent(id)}&store_id=eq.${encodeURIComponent(sid)}&limit=1`);p=Array.isArray(rows)?rows[0]:null}
      if(!isCurrentEdit(win,id))return;
      const el=win.document.getElementById('productBarcode');if(el)el.value=String(p?.barcode||'');
  ...
  function wrapProductFunctions(win){
    ...
    if(typeof win.closeProductModal==='function'&&!win.closeProductModal.__mwBarcodeFocus){
      ...
      const wrapped=function(){const r=original.apply(this,arguments);resetSearchInput(win,{focus:true});return r};
    ...
    if(typeof win.editProduct==='function'&&!win.editProduct.__mwBarcode){
      ...
      const wrapped=function(id){const r=original.apply(this,arguments);hydrateBarcodeForEdit(win,id);setTimeout(()=>hydrateBarcodeForEdit(win,id),120);setTimeout(()=>hydrateBarcodeForEdit(win,id),450);return r};
    ...
    if(typeof win.loadProducts==='function'&&!win.loadProducts.__mwBarcode){
      ...
      const wrapped=async function(){const r=await original.apply(this,arguments);win.__mwBarcodeProductsCacheAt=0;await refreshProductDecorations(win,true);return r};
      wrapped.__mwBarcode=true;win.loadProducts=wrapped;
    }
  }

  async function handleSearch(win){
    const d=win.document,input=d.getElementById('vendorSmartProductSearch'),value=String(input?.value||'').trim();if(!value){refocusSearchInput(win);return}
    const sid=storeId(win);if(!sid)throw new Error('تعذر تحديد المتجر الحالي.');
    const exact=await rest(win,`local_products?select=id,product_name,barcode,stock_quantity&store_id=eq.${encodeURIComponent(sid)}&barcode=eq.${encodeURIComponent(value)}&limit=1`),byBarcode=Array.isArray(exact)?exact[0]:null;
    ...
    if(looksBarcode){resetSearchInput(win);win.openProductModal?.();setTimeout(()=>{const el=d.getElementById('productBarcode');if(el){el.value=value;el.focus()}},0);return}
    if(Array.isArray(byName)&&byName.length>1){const hint=d.getElementById('vendorSmartProductSearchHint');if(hint)hint.innerHTML='نتائج متعددة معروضة في الجدول: '+byName.map(p=>esc(p.product_name)).join(' • ');refocusSearchInput(win);return}
    const hint=d.getElementById('vendorSmartProductSearchHint');if(hint)hint.textContent='لا يوجد منتج مطابق بهذا الاسم.';refocusSearchInput(win);
  }
