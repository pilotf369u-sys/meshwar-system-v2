...
(function(){
  ...
  function bindSaveGuard(){
    ...
    document.addEventListener('change',e=>{if(e.target?.id==='mwProductMainCategory'||e.target?.id==='mwProductSubCategory')window.__mwTaxonomyTouchedV10=true},true);
    document.addEventListener('click',e=>{
      if(e.target?.closest?.('#addNewProductBtn,button[onclick="openProductModal()"]'))window.__mwTaxonomyTouchedV10=true;
      if(e.target?.closest?.('button[onclick^="editProduct("]'))window.__mwTaxonomyTouchedV10=false;
      const save=e.target?.closest?.('button[onclick*="saveProduct"]');if(!save)return;
      const store=sessionStore(),id=String(document.getElementById('productId')?.value||'').trim(),name=String(document.getElementById('productName')?.value||'').trim(),taxonomy=taxonomySnapshot();
  ...
  function startVendor(){
    bindSaveGuard();markSaveFunction();let tries=0;const timer=setInterval(()=>{markSaveFunction();if(++tries>=120)clearInterval(timer)},50);
  }
  if(vendorPage)startVendor();
  window.MeshwarTaxonomyPersistenceV10={persistTaxonomy,hydrateTaxonomyForEdit,taxonomySnapshot};
})();
