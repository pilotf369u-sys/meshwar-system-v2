...
(function(){
  ...
  const LABELS={color:'اللون',size:'المقاس',volume:'الحجم'};
  ...
  async function enhanceModal(productId){
    ...
    const options=parse(p.options),lists=optionLists(options),matrix=cleanMatrix(options.matrix_stock),variantStock=parse(options.variant_stock),total=p.stock_quantity===null||p.stock_quantity===undefined||p.stock_quantity===''?99:Math.max(0,Math.floor(Number(p.stock_quantity)||0));
  ...
  function renderEditor(seed){const box=document.getElementById('mwMatrixStockEditor');if(!box)return;const lists=formLists(),rows=combinations(lists),prev=seed?cleanMatrix(seed):editorMatrix();if(rows.length===0){box.innerHTML='<div class="mw-matrix-title">مخزون التركيبات</div><div class="mw-matrix-help">أدخل خيارين على الأقل (مثل لون + مقاس) لإنشاء مصفوفة التركيبات.</div>';return}box.innerHTML=`<div class="mw-matrix-title">مخزون التركيبات المترابطة</div><div class="mw-matrix-help">أدخل كمية كل تركيبة. اترك الحقل فارغًا للرجوع إلى مخزون الخيارات/المخزون الإجمالي، أو أدخل 0 لتعطيل هذه التركيبة.</div><div class="mw-matrix-grid">${rows.map(combo=>{const key=matrixKey(combo),label=activeDimensions(lists).map(k=>`${LABELS[k]}: ${combo[k]}`).join(' + ');return `<label class="mw-matrix-row"><span class="mw-matrix-name">${esc(label)}</span><input type="number" min="0" step="1" class="field mw-matrix-input" data-matrix-key="${esc(key)}" value="${Object.prototype.hasOwnProperty.call(prev,key)?prev[key]:''}" placeholder="—"></label>`}).join('')}</div>`}
  ...
  async function seedEditor(productId){if(!productId)return renderEditor({});try{const rows=await rest(`local_products?select=options&id=eq.${encodeURIComponent(productId)}&limit=1`),p=Array.isArray(rows)?rows[0]:null,o=parse(p?.options);renderEditor(o.matrix_stock)}catch(e){console.warn('Matrix stock seed failed',e)}}
  function injectMatrix(body,matrix){const patch=item=>{if(!item||typeof item!=='object')return item;const options=parse(item.options);item.options={...options,matrix_stock:matrix};return item};return Array.isArray(body)?body.map(x=>patch({...x})):patch({...body})}
  function wrapVendorSave(){if(typeof window.saveProduct!=='function'||window.saveProduct.__mwMatrixWrapped)return false;const original=window.saveProduct;const wrapped=async function(...args){const matrix=editorMatrix(),realFetch=window.fetch;window.fetch=async function(input,init={}){try{const url=typeof input==='string'?input:String(input?.url||''),method=String(init.method||'GET').toUpperCase();if(url.includes('/rest/v1/local_products')&&(method==='POST'||method==='PATCH')&&init.body){const parsed=JSON.parse(init.body);init={...init,body:JSON.stringify(injectMatrix(parsed,matrix))}}}catch(e){console.warn('Matrix stock request injection skipped',e)}return realFetch.call(this,input,init)};try{return await original.apply(this,args)}finally{window.fetch=realFetch}};wrapped.__mwMatrixWrapped=true;wrapped.__mwVariantWrapped=original.__mwVariantWrapped;wrapped.__mwMulti=original.__mwMulti;window.saveProduct=wrapped;return true}
