/* MeshWar Customer Featured Hub v1.0 — reads active_merchant_slots_view only. */
(()=>{
'use strict';
...
function mount(){
  if(document.getElementById('meshwarFeaturedHub'))return document.getElementById('meshwarFeaturedHub');
  const oldVideo=document.getElementById('meshwarVideoAd');
  const oldBrand=document.querySelector('.header-container');
  const anchor=oldVideo||oldBrand||document.getElementById('meshwarTicker')||document.getElementById('meshwarStoreTabs');
  const hub=document.createElement('section');hub.id='meshwarFeaturedHub';hub.innerHTML='<div class="mw-featured-empty">جاري تحميل المتاجر المتميزة والإعلانات...</div>';
  if(anchor?.parentNode)anchor.parentNode.insertBefore(hub,anchor);else document.body.appendChild(hub);
  /* Cleanup obsolete promo/logo region after preserving insertion point. */
  oldVideo?.remove();oldBrand?.remove();
  return hub;
}
async function load(){const hub=mount();try{
  ...
  if(!slots.length){hub.innerHTML='<div class="mw-featured-empty">لا توجد حملات مميزة نشطة حالياً.</div>';return}
  ...
  const heroHtml=`<div class="mw-featured-head"><div><div class="mw-featured-kicker">اختيار MeshWar</div><h2>المتجر المميز</h2></div></div><div class="mw-featured-hero"><div class="mw-featured-store"><img class="mw-featured-logo" src="${esc(logo(hero))}" alt="${esc(storeName(hero))}" onerror="this.onerror=null;this.src='${esc(placeholder)}'"><h3>${esc(storeName(hero))}</h3><span class="mw-featured-badge">★ اشتراك مميز</span></div><div class="mw-scroll-row">${heroProducts.length?heroProducts.map(p=>productCard(p)).join(''):'<div class="mw-featured-empty">منتجات المتجر ستظهر هنا عند إضافتها.</div>'}</div></div>`;
  const storesHtml=`<div class="mw-featured-section"><div class="mw-featured-head"><h2>متاجر محلية متميزة</h2><span class="mw-featured-kicker">مختارة لك</span></div><div class="mw-featured-grid">${featured.length?featured.map(storeCard).join(''):'<div class="mw-featured-empty">لا توجد متاجر مميزة إضافية حالياً.</div>'}</div></div>`;
  const sponsoredHtml=`<div class="mw-featured-section"><div class="mw-featured-head"><h2>منتجات مدعومة</h2><span class="mw-featured-kicker">إعلانات</span></div><div class="mw-scroll-row">${sponsored.length?sponsored.slice(0,20).map(p=>productCard(p,true)).join(''):'<div class="mw-featured-empty">لا توجد منتجات إعلانية نشطة حالياً.</div>'}</div></div>`;
  hub.innerHTML=heroHtml+storesHtml+sponsoredHtml;
}catch(e){console.error('Featured merchant hub load error',e);hub.innerHTML='<div class="mw-featured-empty mw-featured-error">تعذر تحميل المتاجر المتميزة حالياً.</div>'}}
