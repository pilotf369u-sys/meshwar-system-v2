...
(function(){
  ...
  async function rpc(win,name,body){
    const r=await win.fetch(`${SB_URL}/rest/v1/rpc/${name}`,{method:'POST',cache:'no-store',headers:{apikey:SB_KEY,Authorization:`Bearer ${SB_KEY}`,'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify(body)});
  ...
  async function submit(win,e){
    ...
    try{
      ...
      await rpc(win,'vendor_add_operating_expense',{p_store_id:String(st.id),p_amount:amount,p_currency:cur,p_category:category||'تشغيلي',p_note:note||null,p_expense_date:new Date().toISOString().slice(0,10)});
      ...
      await win.parent?.MeshwarVendorFinanceV21?.refresh?.(win,true).catch(()=>{});
      if(win.MeshwarVendorFinanceV21?.refresh)await win.MeshwarVendorFinanceV21.refresh(win,true);
  ...
  window.MeshwarVendorPlExpenseV23={install,submit,VERSION};
})();
