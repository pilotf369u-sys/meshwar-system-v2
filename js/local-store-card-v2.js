...
async function createOrder(product,store){
  ...
  try{
    ...
    alert('تم إرسال الطلب بنجاح. السعر: '+fmt(usd)+' $ / '+fmt(local)+' '+cur+' — رقم الطلب: '+orderCode);location.href='dashboard.html?customerId='+encodeURIComponent(customer.id);
...
async function renderLocalStoreV2(){
  ...
  try{
    ...
    const cur=curOf(store);grid.innerHTML=(products||[]).map(p=>{const usd=customerUsd(p,store),local=localPrice(usd,store),oldUsd=oldCustomerUsd(p,store),oldLocal=oldUsd===null?null:localPrice(oldUsd,store),unavailable=!!p.is_out_of_stock||usd===null||local===null;const old=oldUsd===null?'':`<span class="local-v2-old">قبل الخصم: ${fmt(oldUsd)} $${cur!=='USD'?' / '+fmt(oldLocal)+' '+esc(cur):''}</span>`;return `<article class="local-v2-card"><div class="local-v2-img-wrap"><img class="local-v2-img" src="${esc(p.image_url||placeholder)}" onerror="this.onerror=null;this.src='${esc(placeholder)}'" alt="${esc(p.product_name||'منتج')}"></div><div class="local-v2-body"><div class="local-v2-name">${esc(p.product_name||'منتج')}</div><div class="local-v2-desc">${esc(p.description||'')}</div><div class="local-v2-price">${unavailable?'غير متوفر حالياً':`${old}<div class="local-v2-usd">${fmt(usd)} $</div><div class="local-v2-local">${fmt(local)} ${esc(cur)}</div><small class="local-v2-note">السعر شامل هامش MeshWar — التوصيل يحدد لاحقاً</small>`}</div><button type="button" class="local-v2-order" ${unavailable?'disabled':''} data-pid="${esc(p.id)}">${unavailable?'غير متوفر حالياً':'🛒 اطلب الآن'}</button></div></article>`}).join('')||'<div class="local-empty">لا توجد منتجات مضافة لهذا المتجر حالياً.</div>';
