...
.local-v3-name{font-size:1.1rem!important;font-weight:700!important;color:#ffffff!important;margin-bottom:6px!important}.local-v3-desc{font-size:.9rem!important;color:#f1f5f9!important;font-weight:500!important;line-height:1.5!important;min-height:22px!important;margin-top:4px!important}
...
async function fetchStoreBundle(storeId){
  ...
  const [stores,products]=await Promise.all([
    ...
    restRequest(`local_products?select=id,product_name,image_url,description,base_price,discount_price,is_out_of_stock,options&store_id=eq.${encoded}&order=created_at.desc`,{timeout:3000})
  ]);
...
function renderCards(grid,store,products,{fromCache=false}={}){
  const cards=(products||[]).map(p=>{
    ...
    return `<article class="local-v3-card">${badge}<div class="local-v3-img-wrap"><img class="local-v3-img" src="${esc(p.image_url||placeholder)}" onerror="this.onerror=null;this.src='${esc(placeholder)}'" alt="${esc(p.product_name||'منتج')}"></div><div class="local-v3-body"><div class="local-v3-name">${esc(p.product_name||'منتج')}</div><div class="local-v3-desc">${esc(p.description||'')}</div><div class="local-v3-price">${unavailable?'غير متوفر حالياً':`${old}<div class="local-v3-local"><span class="local-v3-money" dir="ltr">${esc(iqdLabel(iqd))}</span></div><small class="local-v3-note">السعر شامل هامش MeshWar — التوصيل يحدد لاحقاً</small>`}</div><button type="button" class="local-v3-order" ${unavailable?'disabled':''} data-pid="${esc(p.id)}">${unavailable?'غير متوفر حالياً':'<span aria-hidden="true" style="display:inline-block;line-height:1">🛒</span><span style="display:inline-block;line-height:1.2">اطلب الآن</span>'}</button></div></article>`;
  }).join('');
