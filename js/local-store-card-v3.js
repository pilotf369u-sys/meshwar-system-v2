...
function renderCards(grid,store,products,{fromCache=false}={}){
  ...
  grid.querySelectorAll('.local-v3-order[data-pid]').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    for(const [key,values,label] of [['color',opts.colors,'اللون'],['size',opts.sizes,'المقاس'],['volume',opts.volumes,'الحجم']]){if(!values.length)continue;const select=card?.querySelector(`.local-v3-option-select[data-option="${key}"]`),value=String(select?.value||'').trim();if(!value){alert('يرجى اختيار '+label+' قبل الإضافة للسلة.');select?.focus();return}selection[key]=value}
    const requestedQuantity=Math.max(1,Math.floor(Number(card?.querySelector('[data-qty-value]')?.textContent)||1)),cart=window.KintoLocalCartV93,unitIqd=customerPriceIqd(p,store),oldUnitIqd=oldCustomerPriceIqd(p,store);if(!cart||typeof cart.add!=='function'){alert('سلة المتاجر المحلية غير جاهزة. أعد تحميل الصفحة.');return}if(unitIqd===null){alert('سعر المنتج غير صالح.');return}cart.add({store_id:store.id,store_name:store.store_name||'',product_id:String(p.id),product_name:p.product_name||'',image_url:p.image_url||'',selected_options:selection,quantity:requestedQuantity,unit_price_local:ceilPrice(unitIqd),old_unit_price_local:oldUnitIqd!==null?ceilPrice(oldUnitIqd):null,currency:'IQD'});btn.textContent='✓ تمت الإضافة للسلة';setTimeout(()=>{if(btn.isConnected&&!btn.disabled)btn.textContent='أضف للسلة'},900);
  }));
}
...
async function createOrder(product,store,selection={},requestedQuantity=1){
  ...
  try{
    ...
    await restRequest('orders',{method:'POST',body:[payload],timeout:5000});alert('تم إرسال الطلب بنجاح. الكمية: '+requestedQuantity+' — الإجمالي: '+iqdLabel(totalIqd)+' — رقم الطلب: '+orderCode);location.href='external-shipping-shell.html?screen=customer&customerId='+encodeURIComponent(customer.id);return{ok:true,orderCode,customerId:customer.id};
