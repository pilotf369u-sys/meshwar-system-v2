...
(function(){
  ...
  async function rest(win,path,{method='GET',body=null}={}){
    const r=await win.fetch(`${SB_URL}/rest/v1/${path}`,{
      method,cache:'no-store',headers:{apikey:SB_KEY,Authorization:`Bearer ${SB_KEY}`,'Content-Type':'application/json',Accept:'application/json'},
  ...
  async function decorateBarcodes(win){
    ...
    win.document.querySelectorAll('#productsBody tr').forEach(row=>{
      ...
      if(!label){label=win.document.createElement('div');label.dataset.mwBarcodeLabel='1';label.className='mt-1 text-[11px] font-bold text-amber-300';row.querySelector('td')?.appendChild(label)}
  ...
  async function persistCanonicalBarcode(win,{id,name,barcode}){
    const sid=String(store(win)?.id||'').trim(),code=String(barcode||'').trim();if(!sid||!code)return;
    const productId=await resolveProductId(win,{id,name,storeId:sid});if(!productId)return;
    await rest(win,`local_products?id=eq.${q(productId)}&store_id=eq.${q(sid)}`,{method:'PATCH',body:{barcode:code}});
    win.__mwBarcodeProductsCacheAt=0;setTimeout(()=>decorateBarcodes(win).catch(()=>{}),120);
  }
  function bindBarcodeSaveGuard(win){
    if(win.__mwV22BarcodeSaveGuard)return;
    win.document.addEventListener('click',e=>{
      const btn=e.target?.closest?.('button');if(!btn)return;
      const onclick=String(btn.getAttribute('onclick')||''),text=String(btn.textContent||'');if(!onclick.includes('saveProduct')&&!text.includes('حفظ المنتج'))return;
      const id=String(win.document.getElementById('productId')?.value||'').trim(),name=String(win.document.getElementById('productName')?.value||'').trim(),barcode=String(win.document.getElementById('productBarcode')?.value||'').trim();if(!barcode)return;
      const persist=()=>persistCanonicalBarcode(win,{id,name,barcode}).catch(err=>console.warn('V22 barcode save guard failed',err));
      if(id)persist();else[180,500,1100].forEach(ms=>setTimeout(persist,ms));
    },true);
    win.__mwV22BarcodeSaveGuard=true;
  }

  async function findByCode(win,value){
    const sid=String(store(win)?.id||'').trim();if(!sid)return null;
    let rows=await rest(win,`local_products?select=id,product_name,barcode,stock_quantity&store_id=eq.${q(sid)}&barcode=eq.${q(value)}&limit=1`)||[];
    if(rows[0])return rows[0];
    try{rows=await rest(win,`local_products?select=id,product_name,barcode,sku,stock_quantity&store_id=eq.${q(sid)}&sku=eq.${q(value)}&limit=1`)||[];return rows[0]||null}catch{return null}
  }

  async function handleSmartSearch(win,e){
    if(e.key!=='Enter')return;
    const input=e.currentTarget,value=String(input.value||'').trim();if(!value)return;
    ...
    try{
      ...
      if(/^[A-Za-z0-9._-]{4,80}$/.test(value)){input.value='';win.openProductModal?.();setTimeout(()=>{const el=win.document.getElementById('productBarcode');if(el){el.value=value;el.focus()}},0);return}
      const hint=win.document.getElementById('vendorSmartProductSearchHint');if(hint)hint.textContent=byName.length?'نتائج متعددة معروضة في الجدول.':'لا يوجد منتج مطابق بهذا الاسم.';
    }catch(err){console.error('V22 barcode search failed',err);win.alert?.('تعذر تنفيذ البحث: '+(err?.message||err))}
  }
  ...
  function bindEditCapture(win){
    ...
    win.document.addEventListener('click',e=>{const btn=e.target?.closest?.('button[onclick*="editProduct("]');if(!btn)return;const code=String(btn.getAttribute('onclick')||''),id=code.match(/editProduct\(['"]([^'"]+)['"]\)/)?.[1]||'';if(id)scheduleBarcodeHydrate(win,id)},true);
    win.__mwV22EditCapture=true;
  }

  function autoPrice(win,{force=false}={}){
    const costEl=win.document.getElementById('mwProductCostPrice'),baseEl=win.document.getElementById('productBasePrice'),marginEl=win.document.getElementById('mwGlobalProfitMargin');if(!costEl||!baseEl||!marginEl)return;
    const raw=String(costEl.value||'').trim(),marginRaw=String(marginEl.value||'').trim();if(raw===''||marginRaw==='')return;
    const cost=Number(raw),margin=Number(marginRaw);if(!Number.isFinite(cost)||cost<0||!Number.isFinite(margin)||margin<0)return;if(win.__mwV22ManualBase&&!force)return;
    const price=Math.ceil(cost*(1+margin/100));win.__mwV22InternalPrice=true;baseEl.value=String(price);baseEl.dispatchEvent(new win.Event('input',{bubbles:true}));win.__mwV22InternalPrice=false;
    const hint=win.document.getElementById('mwAutoPriceHint');if(hint)hint.textContent=`سعر البيع المقترح: ${price} (تكلفة ${cost} + ربح ${margin}%)`;
  }
  function bindProductPricing(win){
    const d=win.document,cost=d.getElementById('mwProductCostPrice'),base=d.getElementById('productBasePrice');if(!cost||!base)return;
    if(!d.getElementById('mwAutoPriceHint')){const h=d.createElement('div');h.id='mwAutoPriceHint';h.className='text-[11px] font-bold text-emerald-300';cost.insertAdjacentElement('afterend',h)}
    if(!cost.__mwV22Cost){cost.addEventListener('input',()=>{win.__mwV22ManualBase=false;autoPrice(win)});cost.__mwV22Cost=true}
    if(!base.__mwV22Base){base.addEventListener('input',()=>{if(!win.__mwV22InternalPrice)win.__mwV22ManualBase=true});base.__mwV22Base=true}
  }

  async function loadMargin(win){
    const st=store(win),el=win.document.getElementById('mwGlobalProfitMargin');if(!st?.id||!el)return;if(st.profit_margin_percent!=null&&el.value==='')el.value=String(st.profit_margin_percent);
    try{const value=await rpc(win,'vendor_get_profit_margin',{p_store_id:String(st.id)}),candidate=Array.isArray(value)?value[0]:value,margin=typeof candidate==='number'?candidate:(typeof candidate==='string'&&candidate.trim()!==''?Number(candidate):NaN);if(Number.isFinite(margin)){el.value=String(margin);st.profit_margin_percent=margin;saveStore(win,st)}}catch(e){console.warn('V22 profit margin hydrate failed',e)}
  }
  async function saveMargin(win){
    const st=store(win),el=win.document.getElementById('mwGlobalProfitMargin');if(!st?.id||!el)return;const margin=Number(el.value);if(!Number.isFinite(margin)||margin<0||margin>10000){win.alert?.('أدخل نسبة ربح صحيحة بين 0 و10000.');return}
    try{await rpc(win,'vendor_set_profit_margin',{p_store_id:String(st.id),p_margin:margin});st.profit_margin_percent=margin;saveStore(win,st);win.showNotice?.('تم حفظ نسبة الربح العامة.');autoPrice(win,{force:!win.__mwV22ManualBase})}catch(e){console.error('V22 margin save failed',e);win.showNotice?.('تعذر حفظ نسبة الربح: '+(e?.message||e),true)}
  }
  function injectMarginUi(win){
    const d=win.document;if(d.getElementById('mwGlobalProfitMargin'))return;const heading=[...d.querySelectorAll('h1,h2,h3')].find(x=>String(x.textContent||'').includes('سعر الصرف المركزي')),card=heading?.closest('.glass'),grid=card?.querySelector('.grid');if(!grid)return;
    const wrap=d.createElement('div');wrap.className='col-span-2 grid grid-cols-[1fr_auto] gap-2';wrap.innerHTML='<input id="mwGlobalProfitMargin" class="field" type="number" min="0" max="10000" step="0.01" placeholder="نسبة الربح العامة %"><button id="mwSaveProfitMargin" type="button" class="rounded-2xl border border-amber-400/40 bg-amber-500/15 px-4 font-black text-amber-200 transition hover:bg-amber-500/25">حفظ نسبة الربح</button>';grid.appendChild(wrap);
  ...
  function boot(win){
    injectMarginUi(win);bindSearch(win);bindEditCapture(win);bindBarcodeSaveGuard(win);bindProductPricing(win);wrapProductFunctions(win);decorateBarcodes(win).catch(()=>{});
