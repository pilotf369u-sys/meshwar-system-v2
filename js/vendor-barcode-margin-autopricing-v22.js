...
(function(){
  ...
  async function decorateBarcodes(win){
    const rows=await fetchBarcodeRows(win);const map=new Map(rows.map(p=>[String(p.id),p]));
    win.document.querySelectorAll('#productsBody tr').forEach(row=>{
      ...
      const id=onclick.match(/editProduct\(['"]([^'"]+)['"]\)/)?.[1]||'';const p=map.get(String(id));if(!p)return;
  ...
  function autoPrice(win,{force=false}={}){
    const costEl=win.document.getElementById('mwProductCostPrice'),baseEl=win.document.getElementById('productBasePrice'),marginEl=win.document.getElementById('mwGlobalProfitMargin');
    if(!costEl||!baseEl||!marginEl)return;
    const raw=String(costEl.value||'').trim(),marginRaw=String(marginEl.value||'').trim();if(raw===''||marginRaw==='')return;
    const cost=Number(raw),margin=Number(marginRaw);if(!Number.isFinite(cost)||cost<0||!Number.isFinite(margin)||margin<0)return;
    if(win.__mwV22ManualBase&&!force)return;
  ...
  async function saveMargin(win){
    const st=store(win),el=win.document.getElementById('mwGlobalProfitMargin');if(!st?.id||!el)return;
    const margin=Number(el.value);if(!Number.isFinite(margin)||margin<0||margin>10000){win.alert?.('أدخل نسبة ربح صحيحة بين 0 و10000.');return}
    try{
      await rpc(win,'vendor_set_profit_margin',{p_store_id:String(st.id),p_margin:margin});st.profit_margin_percent=margin;saveStore(win,st);win.showNotice?.('تم حفظ نسبة الربح العامة.');autoPrice(win,{force:!win.__mwV22ManualBase});
    }catch(e){console.error('V22 margin save failed',e);win.showNotice?.('تعذر حفظ نسبة الربح: '+(e?.message||e),true)}
  }
