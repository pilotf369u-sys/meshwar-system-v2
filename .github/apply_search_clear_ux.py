...
for fn,cfg in FILES.items():
    ...
    # Add a clear button immediately after the search input, if absent.
    if f'data-clear-search="{iid}"' not in s:
        # Match exact input tag by id without touching any other controls.
    ...
    # Insert generic clear + scanner-burst helpers immediately before cleanBarcodeScannerInput.
    ...
    # Wire the search input keydown through burst-preparation while preserving existing handler.
    # Only append prepare helper if not already present on the input.
    ...
    # Auto-clear old search at the beginning of camera scan. Delivery has its own startCameraScanner,
    # other dashboards share the same function name after the previous fix.
    ...
    # Export clear helper when the file uses Object.assign(window,...), otherwise inline onclick resolves globally.
    # Function declarations are global in classic scripts; no further changes required.
