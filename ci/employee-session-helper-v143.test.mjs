...
sessions.save({token:'logout-even-on-error',expiresAt:new Date(Date.now()+3600000).toISOString()});
await assert.rejects(
  sessions.logout({rpc:async()=>{throw new Error('network failure')}})
);
assert.equal(sessions.read(),null,'logout must clear the browser session even when server revocation fails');
