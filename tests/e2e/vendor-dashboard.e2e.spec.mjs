...
test.describe('MeshWar vendor E2E integration gate',()=>{
  ...
  test('orders: lifecycle rendering, filters, barcode search, camera, details and label print',async({page})=>{
    ...
    await expect(vendor.locator('#ordersBody')).toContainText('قيد التوصيل');
  ...
  test('catalog: add/edit stock, taxonomy persistence and pagination',async({page})=>{
    ...
    await expect(vendor.locator('#mwProductMainCategory')).toBeAttached();
    ...
    await expect(vendor.locator('#mwProductSubCategory option[value="sub-a"]')).toHaveCount(1);
    ...
    await expect(vendor.locator('#mwProductSubCategory option[value="sub-b"]')).toHaveCount(1);
    ...
    await expect.poll(()=>frameWindow(page,()=>window.__MESH_E2E_DB.local_products.find(p=>p.id==='p-1')?.subcategory_id||'')).toBe('sub-b');
  ...
  test('UI: real categories pagination, single active tab and theme consistency',async({page})=>{
    const vendor=await openVendor(page);
    await vendor.locator('#vendorTabBtn-categories').click();
    ...
    await expect(vendor.locator('#mwCatList .mw-cat-group')).toHaveCount(21);
    ...
    await vendor.locator('#mwVendorPager-categories [data-page-action="next"]').click();
    await expect(vendor.locator('#mwVendorPager-categories [data-pager-info]')).toContainText('11–20 من 21');
