...
export function fixtureDb(){
  ...
  const order_store_segments=[{segment_id:'seg-e2e-1',order_id:'global-e2e-1',order_code:'KN-009501',reference_order_no:'SIP-9501',order_created_at:new Date().toISOString(),items_preview:[{product_id:'p-1',product_name:'Segment Product',product_image:'',quantity:2,unit_price_local:8000,line_total_local:16000,currency:'IQD'}],quantity_total:2,subtotal_local:16000,currency:'IQD',store_status:'بانتظار التسديد',confirmed_at:new Date().toISOString(),vendor_payment_status:'pending',customer_location:{country:'العراق',province:'بغداد',city:'الكرادة',label:'بغداد / الكرادة'},customer:{name:'عميل الاختبار',code:'CUS-9501',phone:'07700000000',secondary_phone:'07800000000',country:'العراق',province:'بغداد',address:'الكرادة، شارع الاختبار'}}];
  const settings=[{key:'site_settings',value:{support_phone:'00905378240430',support_email:'support@kinto.test',contact_address:'Istanbul'}}];
  const customers=[{id:'e2e-customer',name:'عميل الاختبار',code:'CUS-E2E',phone:'07700000000',country:'العراق'}];
  return{customers,local_products:products,orders,order_store_segments,store_categories:categories,vendor_operating_expenses:[],settings};
}
...
export function createClient(){
  ...
  class Query{
    ...
    async maybeSingle(){const result=await this.exec();return{...result,data:Array.isArray(result.data)?(result.data[0]||null):result.data}}
    async single(){const result=await this.maybeSingle();return result.data?result:{data:null,error:{message:'ROW_NOT_FOUND'}}}
  ...
  return{from:table=>new Query(table),rpc:async(name,args)=>{(globalThis.__MESH_E2E_RPC_CALLS||(globalThis.__MESH_E2E_RPC_CALLS=[])).push({name,args});if(name==='customer_session_identity_v150')return{data:{ok:true,customer:db.customers[0]},error:null};if(name==='customer_review_login_v132')return{data:{ok:true,session_token:'e2e-review-token',expires_at:new Date(Date.now()+3600000).toISOString()},error:null};if(name==='customer_review_ready_products_v132')return{data:{items:[{order_id:'o-paid',product_id:'p-1',product_name:'Test Product 01',product_image:'',ready_for_review:true,review_id:null}],ready_count:1},error:null};if(name==='vendor_login')return{data:globalThis.__MESH_E2E_STORE,error:null};if(name==='vendor_login_session')return{data:{store:globalThis.__MESH_E2E_STORE,session_token:'e2e-secure-token',expires_at:new Date(Date.now()+3600000).toISOString()},error:null};if(name==='vendor_list_order_segments')return{data:db.order_store_segments.map(({customer,...row})=>row),error:null};if(name==='vendor_get_order_segment_details'){const row=db.order_store_segments.find(x=>x.segment_id===args.p_segment_id);return{data:row?{...row,items:row.items_preview,customer_snapshot:row.customer}:null,error:row?null:{message:'ORDER_SEGMENT_NOT_FOUND'}}}if(name==='vendor_advance_order_segment_status'){const row=db.order_store_segments.find(x=>x.segment_id===args.p_segment_id);if(!row||row.store_status!==args.p_expected_status)return{data:null,error:{message:'ORDER_STATUS_CONFLICT'}};row.store_status=args.p_next_status;globalThis.__MESH_E2E_CANONICAL_STATUS={order_id:row.order_id,status:args.p_next_status};const order=db.orders.find(x=>x.id===row.order_id);if(order)order.status=args.p_next_status;return{data:{segment_id:row.segment_id,order_id:row.order_id,store_status:row.store_status,canonical_order_status:row.store_status},error:null}}return{data:{ok:true,args},error:null}},storage:{from:()=>({upload:async()=>({error:null}),getPublicUrl:()=>({data:{publicUrl:'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=='}})})},channel:()=>({on(_event,filter,handler){if(filter?.table==='orders')globalThis.__MESH_E2E_ORDERS_REALTIME=handler;return this},subscribe(){return this}}),removeChannel:async()=>{}};
}
`;
