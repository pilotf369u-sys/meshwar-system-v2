...
export function createClient(){
  ...
  class Query{
    ...
    select(){this.mode=this.mode==='select'?'select':this.mode;return this}
    ...
    async exec(){
      ...
      if(this.mode==='insert'){
        ...
        const added=this.payload.map((r,i)=>({...r,id:r.id||'e2e-'+Date.now()+'-'+i,created_at:r.created_at||now,updated_at:r.updated_at||now}));
        rows.push(...added);return{data:added,error:null};
      }
      ...
      let out=[...selected];
      if(this.orderBy){const[k,asc]=this.orderBy;out.sort((a,b)=>String(a?.[k]??'').localeCompare(String(b?.[k]??''))*(asc?1:-1));}
      if(Number.isFinite(this.max))out=out.slice(0,this.max);
  ...
  return{
    ...
    rpc:async(name,args)=>name==='vendor_login'?{data:globalThis.__MESH_E2E_STORE,error:null}:{data:{ok:true,args},error:null},
    storage:{from:()=>({upload:async()=>({error:null}),getPublicUrl:()=>({data:{publicUrl:'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=='}})})},
...
function initBrowserFixture({db,store}){
  window.__MESH_E2E_DB=JSON.parse(JSON.stringify(db));
  window.__MESH_E2E_STORE=JSON.parse(JSON.stringify(store));
  try{sessionStorage.setItem('meshwar_vendor_store',JSON.stringify(store))}catch{}
  const nativeFetch=window.fetch.bind(window);
  const SB='https://hsmmbloouskqdnptiiad.supabase.co/rest/v1/';
  const applyFilters=(rows,sp)=>{
    let out=[...rows];
    for(const [k,v] of sp.entries()){
      if(['select','order','limit'].includes(k))continue;
      if(v.startsWith('eq.'))out=out.filter(r=>String(r?.[k]??'')===v.slice(3));
      else if(v.startsWith('ilike.')){const needle=v.slice(6).replaceAll('%','').toLowerCase();out=out.filter(r=>String(r?.[k]??'').toLowerCase().includes(needle));}
    }
  ...
  window.fetch=async(input,init={})=>{
    ...
    if(!url.startsWith(SB))return nativeFetch(input,init);
    ...
    if(method==='POST'){let body=JSON.parse(init.body||'[]');body=Array.isArray(body)?body:[body];const now=new Date().toISOString();body=body.map((r,i)=>({...r,id:r.id||'rest-'+Date.now()+'-'+i,created_at:r.created_at||now}));rows.push(...body);return new Response(JSON.stringify(body),{status:201,headers:{'content-type':'application/json'}})}
...
export async function installMocks(page){
  const db=fixtureDb();
  await page.context().addInitScript(initBrowserFixture,{db,store:STORE});
  await page.context().route('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm',route=>route.fulfill({status:200,contentType:'text/javascript',body:mockSupabaseModule}));
  await page.context().route('https://cdn.tailwindcss.com/**',route=>route.fulfill({status:200,contentType:'text/javascript',body:"window.tailwind={config:{}};document.head.insertAdjacentHTML('beforeend','<style>.hidden{display:none!important}.flex{display:flex!important}</style>');"}));
  await page.context().route('https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js',route=>route.fulfill({status:200,contentType:'text/javascript',body:"window.Html5Qrcode=class{constructor(id){this.id=id}async start(camera,config,ok){window.__E2E_CAMERA_START={camera,config};window.__E2E_CAMERA_OK=ok}async stop(){}async clear(){}};"}));
  await page.context().route('https://cdn.jsdelivr.net/npm/jsbarcode@3.11.6/dist/JsBarcode.all.min.js',route=>route.fulfill({status:200,contentType:'text/javascript',body:"window.JsBarcode=function(){return true};window.print=function(){};"}));
  return db;
}
...
export async function frameWindow(page,fn,arg){
  ...
  if(!frame)throw new Error('vendor iframe not found');
